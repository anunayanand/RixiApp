...
router.post("/ambassador/:ambassadorId/approve-withdrawal/:withdrawId", authRole("superAdmin"), async (req, res) => {
  try {
    const { ambassadorId, withdrawId } = req.params;
    const { amount, title, transactionId } = req.body;

    if (!amount || !title || !transactionId) {
    ...
    const withdrawal = ambassador.withdrawals.id(withdrawId);
    if (!withdrawal) {
      return res.status(404).json({ success: false, message: "Withdrawal request not found." });
    }

    if (withdrawal.status !== "Pending") {
      return res.status(400).json({ success: false, message: `This request has already been ${withdrawal.status.toLowerCase()}.` });
    }

    // Update withdrawal details
    withdrawal.status = "Approved";
    withdrawal.amount = approvedAmount;
    withdrawal.title = title;
    withdrawal.transactionId = transactionId;
    withdrawal.date = new Date();

    await ambassador.save();

    return res.json({ success: true, message: "Withdrawal request approved and processed successfully!" });
  } catch (err) {
    console.error("Error in approve-withdrawal:", err);
    return res.status(500).json({ success: false, message: "Server error while approving withdrawal." });
  }
});

// ❌ Reject a withdrawal request
router.post("/ambassador/:ambassadorId/reject-withdrawal/:withdrawId", authRole("superAdmin"), async (req, res) => {
  try {
    const { ambassadorId, withdrawId } = req.params;
    ...
    return res.json({ success: true, message: "Withdrawal request has been rejected." });
  } catch (err) {
    console.error("Error in reject-withdrawal:", err);
    return res.status(500).json({ success: false, message: "Server error while rejecting withdrawal." });
  }
});
